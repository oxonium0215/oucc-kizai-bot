...
impl Handler {
    ...
    async fn handle_component(
        ...
        match interaction.data.custom_id.as_str() {
            ...
            _ => {
                ...
                } else if interaction.data.custom_id.starts_with("maint_new_") {
                    self.handle_maintenance_new(ctx, interaction).await?
                } else if interaction.data.custom_id.starts_with("maint_edit_") {
    ...
    async fn handle_modal(
        ...
        match interaction.data.custom_id.as_str() {
            ...
            _ => {
                ...
                } else if interaction.data.custom_id.starts_with("maintenance_modal:") {
                    self.handle_maintenance_modal_submit(ctx, interaction).await?
                } else {
    ...
    async fn handle_maintenance_new(
        &self,
        ctx: &Context,
        interaction: &ComponentInteraction,
    ) -> Result<()> {
        ...
        if !utils::is_admin(ctx, interaction.guild_id.unwrap(), interaction.user.id).await? {
            let response = serenity::all::CreateInteractionResponse::Message(
                serenity::all::CreateInteractionResponseMessage::new()
                    .content("❌ You need administrator permissions to create maintenance windows.")
                    .ephemeral(true),
            );
            interaction.create_response(&ctx.http, response).await?;
            return Ok(());
        ...
        // Extract equipment ID from custom_id (format: maint_new_{equipment_id})
        let equipment_id: i64 = interaction.data.custom_id
            .strip_prefix("maint_new_")
            .unwrap()
            .parse()
            .unwrap();

        // Create modal for maintenance window creation
        use serenity::all::{CreateModal, CreateInputText, InputTextStyle};
        
        let modal = CreateModal::new(format!("maintenance_modal:{}", equipment_id), "Schedule Maintenance")
            .components(vec![
                serenity::all::CreateActionRow::InputText(
                    CreateInputText::new(InputTextStyle::Short, "start_time", "Start Time (JST)")
                        .placeholder("YYYY-MM-DD HH:MM (e.g., 2024-01-15 09:00)")
                        .required(true)
                        .max_length(16)
                ),
                serenity::all::CreateActionRow::InputText(
                    CreateInputText::new(InputTextStyle::Short, "end_time", "End Time (JST)")
                        .placeholder("YYYY-MM-DD HH:MM (e.g., 2024-01-15 17:00)")
                        .required(true)
                        .max_length(16)
                ),
                serenity::all::CreateActionRow::InputText(
                    CreateInputText::new(InputTextStyle::Short, "reason", "Reason (Optional)")
    ...
    async fn handle_maintenance_edit(
        &self,
        ctx: &Context,
        interaction: &ComponentInteraction,
    ) -> Result<()> {
        ...
        if !utils::is_admin(ctx, interaction.guild_id.unwrap(), interaction.user.id).await? {
            let response = serenity::all::CreateInteractionResponse::Message(
                serenity::all::CreateInteractionResponseMessage::new()
                    .content("❌ You need administrator permissions to edit maintenance windows.")
                    .ephemeral(true),
            );
            interaction.create_response(&ctx.http, response).await?;
            return Ok(());
        ...
        // Extract maintenance ID from custom_id (format: maint_edit_{maintenance_id})
        let maintenance_id: i64 = interaction.data.custom_id
            .strip_prefix("maint_edit_")
            .unwrap()
            .parse()
            .unwrap();
        ...
        let maintenance_row = sqlx::query(
            "SELECT equipment_id, start_utc, end_utc, reason FROM maintenance_windows 
        ...
        let Some(maintenance) = maintenance_row else {
            let response = serenity::all::CreateInteractionResponse::Message(
                serenity::all::CreateInteractionResponseMessage::new()
                    .content("❌ Maintenance window not found or already canceled.")
                    .ephemeral(true),
            );
            interaction.create_response(&ctx.http, response).await?;
            return Ok(());
        ...
        let start_utc = Self::naive_datetime_to_utc(maintenance.get("start_utc"));
        let end_utc = Self::naive_datetime_to_utc(maintenance.get("end_utc"));
        let start_jst = crate::time::utc_to_jst_string(start_utc);
        let end_jst = crate::time::utc_to_jst_string(end_utc);
        let current_reason: Option<String> = maintenance.get("reason");

        // Extract just the date and time part (remove seconds if present)
        let start_formatted = if start_jst.len() > 16 { &start_jst[..16] } else { &start_jst };
        let end_formatted = if end_jst.len() > 16 { &end_jst[..16] } else { &end_jst };

        // Create modal for maintenance window editing
        use serenity::all::{CreateModal, CreateInputText, InputTextStyle};
        
        let modal = CreateModal::new(format!("maintenance_modal:edit:{}", maintenance_id), "Edit Maintenance")
            .components(vec![
                serenity::all::CreateActionRow::InputText(
                    CreateInputText::new(InputTextStyle::Short, "start_time", "Start Time (JST)")
                ...
                serenity::all::CreateActionRow::InputText(
                    CreateInputText::new(InputTextStyle::Short, "reason", "Reason (Optional)")
                        .placeholder("e.g., Cleaning, Repair, Inspection")
                        .value(current_reason.unwrap_or_default())
                        .required(false)
                        .max_length(100)
                ),
            ]);
    ...
    async fn handle_maintenance_cancel(
        &self,
        ctx: &Context,
        interaction: &ComponentInteraction,
    ) -> Result<()> {
        ...
        if !utils::is_admin(ctx, interaction.guild_id.unwrap(), interaction.user.id).await? {
            let response = serenity::all::CreateInteractionResponse::Message(
                serenity::all::CreateInteractionResponseMessage::new()
                    .content("❌ You need administrator permissions to cancel maintenance windows.")
                    .ephemeral(true),
            );
            interaction.create_response(&ctx.http, response).await?;
            return Ok(());
        ...
        // Extract maintenance ID from custom_id (format: maint_cancel_{maintenance_id})
        let maintenance_id: i64 = interaction.data.custom_id
            .strip_prefix("maint_cancel_")
            .unwrap()
            .parse()
            .unwrap();

        // Get maintenance details before canceling
        let maintenance_row = sqlx::query(
            "SELECT equipment_id, start_utc, end_utc, reason FROM maintenance_windows 
             WHERE id = ? AND canceled_at_utc IS NULL"
        )
        ...
        let equipment_id: i64 = maintenance.get("equipment_id");
        ...
        let start_jst = crate::time::utc_to_jst_string(start_utc);
        let end_jst = crate::time::utc_to_jst_string(end_utc);
        let reason: Option<String> = maintenance.get("reason");

        let response = serenity::all::CreateInteractionResponse::Message(
            serenity::all::CreateInteractionResponseMessage::new()
                .content(format!(
                    "✅ **Maintenance canceled!**\n\n🔧 **Equipment:** {}\n⏰ **Was scheduled:** {} to {} (JST){}",
                    equipment_name,
                    start_jst,
                    end_jst,
        ...
        // TODO: Trigger reconcile of equipment display to update maintenance status
        // self.reconcile_equipment_display(ctx, guild_id, channel_id).await?;

        Ok(())
    }

    async fn handle_maintenance_modal_submit(
        &self,
        ctx: &Context,
        interaction: &ModalInteraction,
    ) -> Result<()> {
        // Parse custom_id to determine if this is create or edit
        let parts: Vec<&str> = interaction.data.custom_id.split(':').collect();
        if parts.len() < 2 {
            error!("Invalid maintenance modal custom_id: {}", interaction.data.custom_id);
            return Ok(());
        }
        ...
        let reason_opt = if reason.trim().is_empty() { None } else { Some(reason.trim().to_string()) };
                ...
                if let Ok(equipment_id) = equipment_id_str.parse::<i64>() {
                    ...
                    match self.create_maintenance_window_helper(equipment_id, start_utc, end_utc, reason_opt.clone(), user_id).await {
                        Ok(_maintenance_id) => {
                            ...
                            // TODO: Send notifications to affected reservations
                            // TODO: Trigger reconcile of equipment display
    ...
    async fn create_maintenance_window_helper(
        &self,
        equipment_id: i64,
        start_time: chrono::DateTime<chrono::Utc>,
        end_time: chrono::DateTime<chrono::Utc>,
        ...
        // Use the maintenance helper to create the window
        let maintenance_helper = crate::maintenance::MaintenanceHelper::new(self.db.clone());
        maintenance_helper.create_maintenance_window(equipment_id, start_time, end_time, reason, created_by_user_id).await
    }

    async fn update_maintenance_window_helper(
        &self,
        maintenance_id: i64,
        start_time: chrono::DateTime<chrono::Utc>,
        end_time: chrono::DateTime<chrono::Utc>,
        reason: Option<String>,
        _updated_by_user_id: i64,
    ) -> Result<String, String> {
        // Get current maintenance window to check equipment_id
        let maintenance_row = sqlx::query(
            "SELECT equipment_id FROM maintenance_windows WHERE id = ? AND canceled_at_utc IS NULL"
        )
        .bind(maintenance_id)
        .fetch_optional(&self.db)
        .await
        ...
        let Some(maintenance) = maintenance_row else {
            return Err("Maintenance window not found or already canceled".to_string());
        };

        let equipment_id: i64 = maintenance.get("equipment_id");

        // Check for conflicts with other maintenance windows (excluding this one)
        ...
        let overlaps = sqlx::query(
            "SELECT id, start_utc, end_utc FROM maintenance_windows 
             WHERE equipment_id = ? AND canceled_at_utc IS NULL AND id != ?
             AND start_utc < ? AND end_utc > ?"
        )
        .bind(equipment_id)
        .bind(maintenance_id)
        ...
        if !overlaps.is_empty() {
            ...
            let start_jst = crate::time::utc_to_jst_string(Self::naive_datetime_to_utc(overlap.get("start_utc")));
            let end_jst = crate::time::utc_to_jst_string(Self::naive_datetime_to_utc(overlap.get("end_utc")));
            return Err(format!(
                "Updated maintenance window would overlap with existing maintenance from {} to {}",
                start_jst, end_jst
            ));
        }
        ...
        if !conflicts.is_empty() {
            let conflict = &conflicts[0];
            let start_jst = crate::time::utc_to_jst_string(Self::naive_datetime_to_utc(conflict.get("start_time")));
            let end_jst = crate::time::utc_to_jst_string(Self::naive_datetime_to_utc(conflict.get("end_time")));
            return Err(format!(
                "Updated maintenance window would conflict with existing reservation by <@{}> from {} to {}",
                conflict.get::<i64, _>("user_id"), start_jst, end_jst
            ));
        }
        ...
        .execute(&self.db)
        .await
        .map_err(|e| format!("Failed to update maintenance window: {}", e))?;

        // Get equipment name for return
        let equipment_row = sqlx::query("SELECT name FROM equipment WHERE id = ?")
            .bind(equipment_id)
            .fetch_one(&self.db)
            .await
