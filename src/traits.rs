...
pub trait DiscordApi: Send + Sync {
    ...
    async fn send_channel_message(&self, channel_id: ChannelId, content: &str) -> Result<MessageId>;
    
    /// Edit a message
    async fn edit_message(&self, channel_id: ChannelId, message_id: MessageId, new_content: &str) -> Result<()>;
...
impl DiscordApi for MockDiscordApi {
    async fn send_dm(&self, user_id: UserId, content: &str) -> Result<Option<MessageId>> {
        ...
        self.sent_dms.lock().await.push((user_id, content.to_string()));
        // Return a mock message ID
        Ok(Some(MessageId::new(12345)))
    }
    
    async fn send_channel_message(&self, channel_id: ChannelId, content: &str) -> Result<MessageId> {
        self.channel_messages.lock().await.push((channel_id, content.to_string()));
        // Return a mock message ID
        Ok(MessageId::new(67890))
    }
    
    async fn edit_message(&self, channel_id: ChannelId, message_id: MessageId, new_content: &str) -> Result<()> {
        self.edited_messages.lock().await.push((channel_id, message_id, new_content.to_string()));
        Ok(())
    }
    
    async fn delete_message(&self, channel_id: ChannelId, message_id: MessageId) -> Result<()> {
        self.deleted_messages.lock().await.push((channel_id, message_id));
        Ok(())
    }
    
    async fn respond_to_interaction(&self, interaction_id: &str, response: &str) -> Result<()> {
        self.interaction_responses.lock().await.push((interaction_id.to_string(), response.to_string()));
        Ok(())
    }
}
