...
pub async fn create_transfer_request(
    ...
    if existing.is_some() {
        tx.rollback().await?;
        return Err(anyhow::anyhow!("A pending transfer request already exists for this reservation"));
    }
...
async fn test_transfer_state_machine() {
    ...
    assert!(!is_valid_transfer_transition(&TransferStatus::Pending, &TransferStatus::Pending));
    assert!(!is_valid_transfer_transition(&TransferStatus::Accepted, &TransferStatus::Accepted));
}
...
async fn test_single_pending_transfer_invariant() -> Result<()> {
    ...
    let transfer1 = create_transfer_request(
        &ctx.db,
        reservation.id,
        12345,
        67890,
    ...
    let result = create_transfer_request(
        &ctx.db,
        reservation.id,
        12345,
        11111,
    ...
    assert!(result.unwrap_err().to_string().contains("pending transfer request already exists"));
    ...
    let transfer2 = create_transfer_request(
        &ctx.db,
        reservation.id,
        12345,
        11111,
...
async fn test_transfer_status_validation() -> Result<()> {
    ...
    let transfer = create_transfer_request(
        &ctx.db,
        reservation.id,
        12345,
        67890,
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Invalid transfer status transition"));
...
fn test_transfer_status_conversions() {
    ...
    assert_eq!(TransferStatus::from("Invalid".to_string()), TransferStatus::Pending); // Default
