...
async fn test_e2e_happy_path() -> Result<()> {
    ...
    assert_eq!(
        sony_a7.default_return_location,
        Some("Club Room".to_string())
    );
    ...
    let reservation =
        ReservationBuilder::new(sony_a7.id, user1_id, reservation_start, reservation_end)
    ...
    let dm_result = ctx
        .discord_api
        .send_dm(UserId::new(user2_id as u64), &transfer_message)
        .await?;
    ...
    let reminder_message = format!(
        "📅 リマインダー: 「{}」の貸出期限まで15分です。\n返却時刻: {}",
        sony_a7.name, end_jst
    );

    ctx.discord_api
        .send_dm(UserId::new(user2_id as u64), &reminder_message)
        .await?;
    ...
    ctx.discord_api
        .send_dm(UserId::new(user2_id as u64), &return_confirmation)
        .await?;
    ...
    assert_eq!(
        final_equipment.current_location,
        Some(return_location.to_string())
    );
...
async fn test_message_self_heal_on_restart() -> Result<()> {
    ...
    for msg in &managed_messages {
        let content = match msg.message_type.as_str() {
            ...
            "OverallManagement" => "🔧 Overall Management".to_string(),
            _ => "Unknown message type".to_string(),
        };

        let new_message_id = ctx
            .discord_api
            .send_channel_message(ChannelId::new(msg.channel_id as u64), &content)
            .await?;
...
async fn test_comprehensive_notification_flow() -> Result<()> {
    ...
    // Create reservation
    let reservation =
        ReservationBuilder::new(equipment.id, user_id, reservation_start, reservation_end)
            .build(&ctx.db)
            .await?;
    ...
    let transfer =
        crate::transfer_tests::create_transfer_request(&ctx.db, reservation.id, user_id, 67890)
            .await?;
    ...
    ctx.clock
        .set_time(reservation_end - Duration::minutes(15))
        .await;
    ...
    ctx.clock
        .set_time(reservation_end + Duration::minutes(30))
        .await;
    worker.process_pending_jobs().await?;
