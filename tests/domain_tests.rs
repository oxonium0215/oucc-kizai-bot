...
async fn test_maintenance_window_conflict_detection() -> Result<()> {
    ...
    assert!(conflict.is_some(), "Should detect conflict with maintenance window");
    ...
    assert!(no_conflict.is_none(), "Should not detect conflict outside maintenance window");
    ...
    assert!(adjacent_conflict.is_none(), "Should not conflict with adjacent times");
...
async fn test_maintenance_window_creation_with_overlap_check() -> Result<()> {
    ...
    let result1 = maintenance_helper
        .create_maintenance_window(equipment_id, start1, end1, Some("First maintenance".to_string()), 123)
        .await;
    
    assert!(result1.is_ok(), "First maintenance window should be created successfully");
    ...
    let result2 = maintenance_helper
        .create_maintenance_window(equipment_id, start2, end2, Some("Second maintenance".to_string()), 123)
        .await;
    
    assert!(result2.is_err(), "Overlapping maintenance window should be rejected");
    assert!(result2.unwrap_err().contains("overlap"), "Error should mention overlap");
    ...
    let result3 = maintenance_helper
        .create_maintenance_window(equipment_id, start3, end3, Some("Third maintenance".to_string()), 123)
        .await;
    
    assert!(result3.is_ok(), "Non-overlapping maintenance window should be created successfully");
